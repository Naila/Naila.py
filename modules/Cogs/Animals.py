...
class Animals(Cog):
    ...
    @command(description="Random bears!")
    @checks.bot_has_permissions(embed_links=True, attach_files=True)
    async def bear(self, ctx: Context):
        em = Embed(color=await ctx.guildcolor(), description=await Animal(ctx).fact("bear"))
        em.set_image(url=await Animal(ctx).image("bear"))
        await ctx.reply(embed=em)

    @command(description="Random birds!")
    @checks.bot_has_permissions(embed_links=True, attach_files=True)
    async def bird(self, ctx: Context):
        em = Embed(color=await ctx.guildcolor(), description=await Animal(ctx).fact("bird"))
        em.set_image(url=await Animal(ctx).image("bird"))
        await ctx.reply(embed=em)

    @command(description="Random dolphins!")
    @checks.bot_has_permissions(embed_links=True, attach_files=True)
    ...
    @command(description="Random ducks!")
    @checks.bot_has_permissions(embed_links=True, attach_files=True)
    ...
    @command(description="Random elephants!")
    @checks.bot_has_permissions(embed_links=True, attach_files=True)
    ...
    @command(description="Random foxes!")
    @checks.bot_has_permissions(embed_links=True, attach_files=True)
    async def fox(self, ctx: Context):
        em = Embed(color=await ctx.guildcolor(), description=await Animal(ctx).fact("fox"))
        em.set_image(url=await Animal(ctx).image("fox"))
        await ctx.reply(embed=em)

    @command(description="Random giraffes!")
    @checks.bot_has_permissions(embed_links=True, attach_files=True)
    async def giraffe(self, ctx: Context):
        em = Embed(color=await ctx.guildcolor(), description=await Animal(ctx).fact("giraffe"))
        em.set_image(url=await Animal(ctx).image("giraffe"))
        await ctx.reply(embed=em)

    @command(description="Random hippos!")
    @checks.bot_has_permissions(embed_links=True, attach_files=True)
    ...
    @command(description="Random horses!")
    @checks.bot_has_permissions(embed_links=True, attach_files=True)
    ...
    @command(description="Random killer whales!")
    @checks.bot_has_permissions(embed_links=True, attach_files=True)
    ...
    @command(description="Random koalas!")
    @checks.bot_has_permissions(embed_links=True, attach_files=True)
    async def koala(self, ctx: Context):
        em = Embed(color=await ctx.guildcolor(), description=await Animal(ctx).fact("koala"))
        em.set_image(url=await Animal(ctx).image("koala"))
        await ctx.reply(embed=em)

    @command(description="Random lions!")
    @checks.bot_has_permissions(embed_links=True, attach_files=True)
    async def lion(self, ctx: Context):
        em = Embed(color=await ctx.guildcolor(), description=await Animal(ctx).fact("lion"))
        em.set_image(url=await Animal(ctx).image("lion"))
        await ctx.reply(embed=em)

    @group(aliases=["meow"], invoke_without_command=True, case_insensitive=True, description="Random cats!")
    @checks.bot_has_permissions(embed_links=True, attach_files=True)
    async def cat(self, ctx: Context, *, breed: str = None):
        if not ctx.invoked_subcommand:
            ...
            em = Embed(color=await ctx.guildcolor(), description=details)
    ...
    @command(description="Random pandas!")
    @checks.bot_has_permissions(embed_links=True, attach_files=True)
    async def panda(self, ctx: Context):
        em = Embed(color=await ctx.guildcolor(), description=await Animal(ctx).fact("panda"))
        em.set_image(url=await Animal(ctx).image("panda"))
        await ctx.reply(embed=em)

    @command(description="Random pigs!")
    @checks.bot_has_permissions(embed_links=True, attach_files=True)
    ...
    @command(description="Random red pandas!")
    @checks.bot_has_permissions(embed_links=True, attach_files=True)
    ...
    @command(description="Random sharks!")
    @checks.bot_has_permissions(embed_links=True, attach_files=True)
    async def shark(self, ctx: Context):
        em = Embed(color=await ctx.guildcolor(), description=await Animal(ctx).fact("shark"))
        em.set_image(url=await Animal(ctx).image("shark"))
        await ctx.reply(embed=em)

    @command(description="Random snakes!")
    @checks.bot_has_permissions(embed_links=True, attach_files=True)
    async def snake(self, ctx: Context):
        em = Embed(color=await ctx.guildcolor(), description=await Animal(ctx).fact("snake"))
        em.set_image(url=await Animal(ctx).image("snake"))
        await ctx.reply(embed=em)

    @command(description="Random spiders!")
    @checks.bot_has_permissions(embed_links=True, attach_files=True)
    ...
    @command(description="Random turtles!")
    @checks.bot_has_permissions(embed_links=True, attach_files=True)
    ...
    @group(invoke_without_command=True, case_insensitive=True, description="Random dogs!")
    @checks.bot_has_permissions(embed_links=True, attach_files=True)
